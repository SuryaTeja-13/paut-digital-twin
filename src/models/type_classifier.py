...
reference paper uses. A simple classifier on the global scattering vector
separates the classes far better (≈0.78 balanced, porosity ≈0.88 vs 0.50).
...
Trains on CPU in a few minutes (scattering is fixed; only a logistic regression
...
class TypeClassifier:
    """Wraps the fitted scaler + logistic regression. Loaded by the pipeline."""

    def __init__(self, scaler, clf, classes, scat_params):
        self.scaler, self.clf, self.classes, self.scat_params = scaler, clf, classes, scat_params

    @staticmethod
    def load(path):
        with open(path, "rb") as f:
            d = pickle.load(f)
        return TypeClassifier(d["scaler"], d["clf"], d["classes"], d["scat_params"])

    def predict_features(self, feats):
        """feats: (N, 2C) -> (idx array, prob array (N, n_classes))."""
...
def train_and_save(manifest, out_path, classes=("porosity", "slag"),
    ...
    clf = LogisticRegression(max_iter=3000, class_weight="balanced").fit(scaler.transform(Xtr), ytr)
    ...
    with open(out_path, "wb") as f:
        pickle.dump({"scaler": scaler, "clf": clf, "classes": list(classes),
